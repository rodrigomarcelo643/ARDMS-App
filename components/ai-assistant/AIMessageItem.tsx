      ...
      <View className={`flex-row max-w-[85%] ${item.sender === 'user' ? 'flex-row-reverse' : ''}`}>
        <View className="w-10 h-10 rounded-full items-center justify-center mx-2">
          {item.sender === 'user' ? (
            userAvatar && !avatarError ? (
              ...
              <View className="w-10 h-10 rounded-full bg-[#8C2323] items-center justify-center">
        ...
        <View
          className={`rounded-2xl px-4 py-3 ${
            item.sender === 'user' ? 'bg-[#8C2323] rounded-tr-sm' : 'bg-gray-100 rounded-tl-sm'
          }`}
        >
          <Text className={`text-base ${item.sender === 'user' ? 'text-white' : 'text-gray-800'}`}>
            {item.text}
          </Text>
          <Text className={`text-xs mt-1 ${item.sender === 'user' ? 'text-[#FFB3B3]' : 'text-gray-500'}`}>
